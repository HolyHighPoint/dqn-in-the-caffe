...
double GameDQNBridge::play(double epsilon) {
    ...
    for (int frame = 0; !ale_.game_over(); frame++) {
        const auto current = dqn::PreprocessScreen(ale_.getScreen());
        ...
        if (past.size() < dqn::ICount) {
            ...
            if (update_) {
                const auto trans = ale_.game_over() ? dqn::Trans(input, action, reward, boost::none) : dqn::Trans(input, action, reward, dqn::PreprocessScreen(ale_.getScreen()));
                dqn_.AddTrans(trans);
